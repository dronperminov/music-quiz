...
RadarChart.prototype.GetPercents = function(data) {
	let total = 0
	...
	for (let i = 0; i < data.length; i++) {
		percents[i] = Math.round(data[i].value / total * 1000) / 10
		sum += percents[i]
	}

	percents[data.length - 1] = Math.round((percents[data.length - 1] + 100 - sum) * 10) / 10
	return percents
}
...
RadarChart.prototype.Plot = function(svg, data) {
