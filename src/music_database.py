...
class MusicDatabase:
    ...
    def get_tracks_by_ids(self, track_ids: List[int]) -> List[Track]:
        track_id2track = {track["track_id"]: track for track in self.database.tracks.find({"track_id": {"$in": track_ids}})}
        return [Track.from_dict(track_id2track[track_id]) for track_id in track_ids]
    ...
    def __get_years_artist_ids(self, query: dict) -> Set[int]:
        artist_ids = set()

        for track in self.database.tracks.find({"year": query}, {"artists": 1}):
    ...
    def __get_artists_search_query(self, params: ArtistsSearch, username: Optional[str]) -> dict:
        ...
        if years_query := query.pop("year", {}):
            artist_ids["$in"].append(self.__get_years_artist_ids(years_query))
